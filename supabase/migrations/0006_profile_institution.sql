-- DecIQ — profile institution + extended poll filters
-- Adds profiles.institution, and extends get_case_vote_aggregate to filter by
-- institution (partial match) and doctor name (partial match), and to expose
-- institution in the returned voter objects.
...
begin
  ...
  v_inst_q := nullif(btrim(p_filters ->> 'institution_query'), '');
  v_name_q := nullif(btrim(p_filters ->> 'name_query'), '');
  ...
  with filtered as (
    ...
    where v.case_id = p_case_id
      ...
      and (v_inst_q is null
           or p.institution ilike '%' || v_inst_q || '%')
      and (v_name_q is null
           or p.display_name ilike '%' || v_name_q || '%')
  ),
  totals as (
